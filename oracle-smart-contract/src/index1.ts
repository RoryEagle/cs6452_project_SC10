...
let account = web3.eth.accounts.wallet.add("0x" + "01deee18cdc869745ccf173b3bee8913edf672ef49fc774204aff6a23e284cc9");
...
//---------------------------------------------------------
    ...
    contract_instance!.events["newTreeAdded(address,address,string,uint256)"]()
    .on("connected", function (subscriptionId: any) {
        ...
        let idx = event.returnValues.idx;
        ...
        let temperature = await axios.post(`http://localhost:8080/addTree`, {owner: owner, address: newTree, location: location},
        {
          headers: {
            ...
            .then(async function (response: any) {
                console.log("add tree of idx " + idx);
            ...
            try {
                contract_instance.methods["verifyTree(uint256)"](idx).send({
                    from: account.address,
                    gasPrice: gasPrice!,
                    gas: Math.ceil(1.2 * await contract_instance.methods["verifyTree(uint256)"]
                    (idx).estimateGas({ from: account.address })),
    ...
    .on("connected", function (subscriptionId: any) {
        ...
        .on("data", async function (event: any) {
            ...
            let forSaleList = await axios.get(`http://localhost:8080/loadTreesForSale`)
            .then(async function (response: any) {
                console.log("getforsaleList");
...
// assume account balance is sufficient
    for (const tree of forSaleList)
        try {
            contract_instance.methods["addTreeForSale(address)"](tree).send({
                from: account.address,
                gasPrice: gasPrice!,
                gas: Math.ceil(1.2 * await contract_instance.methods["addTreeForSale(address)"]
                (tree).estimateGas({ from: account.address })),
            }).then(function (receipt: any) {
